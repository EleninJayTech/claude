...
- [ ] **관찰 — 실적용 세션은 대상 워크스페이스 쪽 교차 기록을 빠뜨리기 쉽다**(2026-08-13 `/wrap`에서 발견). 실적용은 주 대상이 **문서 저장소**이고 대상 프로젝트는 "검증 도구"로 다뤄지다 보니, 재적용 2회를 하고도 playground 단위 PROGRESS 3곳과 `docs/INDEX.md`에 `[공통]` 교차가 없었다(직전 v1.13 회차까지는 있었다 — 실적용이라는 새 유형에서만 빠졌다). 이번엔 `/wrap`의 통합 워크스페이스 1단계가 잡아냈지만, 그 세션이 문서 저장소에서 `/wrap`을 돌리지 않았다면 영영 안 남는다. **다음 실적용 때 재발하면** 00 STEP 5 ② 또는 `/wrap`에 "실적용이면 대상 프로젝트에도 교차 한 줄" 한 줄을 배선한다(1회 재발까지는 관찰)
