...
- [ ] **다음 내부 정합성 전수조사: 2027-02경** — `/audit`(분기 1회 또는 개정 몰린 뒤 — 이 repo 전용 프로젝트 스킬. 2026-08-12에 1~4차 수행: 1·2차=전수, 3차=회귀, 4차=앵커 없는 전수). **다음 회차는 회귀 모드로**(4차가 전수였으므로 — DEC-20260812-bsjeong87-14 ①). 예정일이 지나면 /resume가 안내
