...
- [ ] **정기 실적용 검증** — `/audit`(내부 정합성)·`/reverify`(외부 최신성)와 나란한 **셋째 축**. 대량 개정 뒤에는 회귀 감사 다음에 실적용을 한 번 돌린다(DEC-20260812-bsjeong87-16 ①). **다음 실적용: 2026-11경**(또는 대량 개정 직후)
- [ ] **playground 3개 repo 커밋 대기** — 루트(`CLAUDE.md`·`bubble_memo/CLAUDE.md`·`docs/setup_1.md`·`docs/setup_2.md`)·`capi-copy`·`dom_watcher`. 00 STEP 5가 "커밋을 **안내**한다"라 실행하지 않음. 다음 세션이 그 repo에서 `/resume`을 돌리면 git status로 발견된다
