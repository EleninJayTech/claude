...
- [ ] ~~**다음 /audit(회귀 모드): 2026-09경** — 2026-08-20 6문서 대량 개정(00 v1.24~06 v1.9) 직후 필수 절차(DEC-20260812-bsjeong87-15 ① — 권장이 아니라 절차). **2026-08-20 조사·보고까지 수행(확정 34건) — 수정 미착수라 미완**~~: 수정을 마치고 check.ps1 재실행이 통과해야 이 줄을 체크한다(발견 목록은 아래 미해결 항목이 보유)
...
- [ ] ~~🔴 **/audit 6차 확정 34건이 미수정 상태**~~(2026-08-20 조사·보고, 승인 대기 — [[DEC-20260820-bsjeong87-05]]). **[상] 1건은 지금 이 PC에서 작동 중인 결함**이다: `/compact` wrap-gate 통과 조건 `미커밋 0 AND 오늘 기록`이 /wrap 정본(커밋하지 않는다)과 모순 — 이 repo는 커밋 상시 허용이라 무증상이나 **글로벌 hook이라 타 repo에선 정석 순서가 항상 차단**된다(처방 3방향: 좁히기=오늘 기록 단독 판정 / 되돌리기=차단→추천 / 고치기=PROGRESS가 미커밋 목록에 있으면 흔적 인정). 겹치는 [중]: 날짜 판정의 생산자 부재(양식에 날짜 필드 없음). 그 외 [중] 11건 = 03 hud 스코프 검증 무력·03 표시 구성 3택 미배선·06 GitHub App 선설치 게이트 부재·06 §4 게이트 분기 부재·04 제거 OAuth 누락·04 검증 멘션 트리거 미반영·04 §3-B 불릿 귀속/주어·00 §B 04행 낡음·01 §D-3 hooks 예시 드리프트·04 §L GitLab 축 소비자 0·05 글로벌 병기 미인라인, [하] 21건. **수정 착수 시 발견 목록은 PROGRESS 2026-08-20 [audit] 항목이 정본**(요약) — 상세 근거는 그 세션 대화에만 있으므로 재조사가 필요하면 회귀 모드로 다시 돌린다. **+1건(같은 날 /wrap이 적발)**: `scripts/check.ps1:149-151`의 settings 4키 비교가 `ConvertTo-Json -Compress` 문자열이라 **JSON 키 순서 변화를 실차이로 오검출**한다(`/model` 전환 등으로 정본이 재직렬화되면 발생 — 이번에 실제로 `settings.hooks 실차이` FAIL, 내용은 동일). 정규화(키 정렬) 비교로 교체 권장 — 01 §D-3 ⑤가 deny 배열에 대해 이미 정한 "순서 무관"의 키 버전이다.
