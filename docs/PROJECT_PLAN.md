...
- [ ] **정기 실적용 검증** — `/audit`(내부 정합성)·`/reverify`(외부 최신성)와 나란한 **셋째 축**. 대량 개정 뒤에는 회귀 감사 다음에 실적용을 한 번 돌린다(DEC-20260812-bsjeong87-16 ①). **다음 실적용: 2026-11경**(또는 대량 개정 직후). 2026-08-12에 **2회차** 수행 — 1회차 4건·2회차 5건으로 두 번 다 수확이 있었고, 2회차는 **직전 커밋이 아직 어떤 감사도 통과하지 않았다**는 이유로 6차 감사 대신 선택했다(DEC-20260812-bsjeong87-17). 대량 개정 직후엔 감사보다 실적용이 먼저다
