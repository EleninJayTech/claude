...
> **문서 버전: v1.90** · 최종 갱신: **2026-09-23** · **최근 재검증: 2026-09-23** · 기준: Claude Code v2.1.280 (Opus 5.5 · Sonnet 5 · Fable 5.1)
>
> | 버전 | 날짜 | 변경 내용 |
> | --- | --- | --- |
> | v1.0~v1.69 | 2026-07-03~2026-09-10 | 8개 소스 통합 초판 → 공식 재검증 반복(deny·샌드박스·`attribution`·`effort`) → §D-6 `/effort`·절약 프로필·§0 기본 원칙·문서 번호 재편 → 부분 선택+§J-1 auto mode → **기록 체계**(/resume 원격 최신화 선행·PROGRESS 읽기 상한·아카이브 포인터 최상단·`[Pending]` 승격·정기 항목 `다음 ○○: YYYY-MM경`) → **점검 스킬 배포**(`/dropin-check`·`/dropin-update`, 스코프 우선순위 **enterprise > personal > project** 정정) → 점검·최신화 스킬 **설치 유형 전 지원**(`출처=` 판별)·/wrap 미러 대조 배선(4키 판정), 커스텀 /resume·/wrap+`docs/` 기록 체계는 **내장 Tasks 대비 정본 유지**, §F-2 A/B/C 폴백 의무화(비-git·`docs/` 부재·단위 없음), §B 혼합형·루트 라우터 예외. → §A STEP 4 **§검증 신설**(글로벌 스킬+프로젝트 산출물 두 축·글로벌 축 실측·비대화형 대체 증거) → §E-2 하위 적용 단위별 질문·§E-3 단위 도출 기준·§E-4 팀원 층위 혼재 규칙 → §F-3 `additionalDirectories` 상대 경로만 → §D-3 hooks 실물 동기(정본 `global-config/settings.json`·`manual` 게이트) → **/resume 읽기 단위 줄 수→최근 ~5항목**·아카이브 임계 크기 축(~120KB)·예정일 안내 미완료만 → 전면 재검증(deny 우회·샌드박스 Windows 미지원·auto-memory 한도·Read deny→Write 차단·auto mode 기본·`sandbox.credentials`) → §J **격리 3단** → **다인 축 배선**(R21~R24 — 동시성 3분기·갈래·날짜·`[Reverted]`·union 금지)·sfa형 반영(B·C 템플릿·C ⊃ B ⊃ A)·최적화 수회. → 단독 적용 시에도 적용 기록·감지 순서 통일. → v1.52 /audit 7차(A wrap에 갈래·날짜·되돌림 양식·기록 3종 경로 기준 판정·`[Pending]` 종결자·동시성 3분기 동기) → v1.53 동명 스킬 메뉴 중복은 B·C형 정상 → v1.54 §F-1 산출물 언어 → v1.55 §D-2 §5 간결은 분량에만(정중체 유지) → v1.56 인자 키 `argument-hint`·`arguments` → v1.57 deny 경로 앵커 슬래시 규칙 → v1.58 스코프 3방향 대비·§D-5 `/context` 축 → v1.59 `SKILL.md` BOM 금지·§L 자기 점검 → v1.60 **§D-7 프롬프트 캐시 친화 운영 신설** → v1.61 `allowed-tools` 신뢰 주의 → v1.62 `.gitignore`에 `.claude/worktrees/` → v1.63 이어받을 것은 항목 앞쪽·PROJECT_PLAN에(R34) → v1.64 /audit 8차(§D-7 포인터·`/clear`는 `/wrap` 뒤·`SessionStart` clear 매처·R34 조건절·배치·종결자) → v1.65 전면 재검증(v2.1.258 — `modelSettings` 모델별 저장·MCP 제공형 플러그인만 캐시 파괴·확장 키 7종·`allowed-tools`는 제한 아님) → v1.66 최적화(R20) → v1.67 스킬 이름 별칭 적용 규칙(폴더명+`name:`·원본 raw 경로 불변·치환 규칙) → v1.68 동명 스킬 계보 판정·§E-4 소유 축·§D-2 §0 우선순위·말투 평서체(R36)·아카이브 즉시 푸시·조건부 예정일 당김 → v1.69 🔴 **배포 저장소 자족성**(미러 정본 분기 제거·§D-2 골격 §9·미러 대조는 유지보수자 전용). 행별 상세는 저장소 git 이력 |
...
> | v1.82 | 2026-09-17 | **사실 정정**(2026-09-17 공식 조회) — 푸터 Fable 5.1 최소 버전 v2.1.255→**v2.1.257** · §D-7 캐시 읽기 단가에 Fable 5.1 2.5% 예외 · Fable 5.1 effort 캐시 예외의 **제외 환경**(Bedrock·Google Cloud·apps 게이트웨이·실험 베타 비활성·HIPAA) 명시 |
> | v1.83 | 2026-09-17 | **/audit 15차(회귀) 반영** — STEP 3 제거 절차를 좁힘(ⓑⓔ는 넣은 배열 항목만, ⓓ는 만든 파일만·쌓인 기록은 기본 보존·`docs/`는 비었을 때만, 전역 주의에 ⓒ 포함, 제거 후 기록은 00 질문 ②의 문서/항목 구분), §검증 ⓕ는 예시가 아니라 확인한 가용 모델 ID 키로·ⓐ `/context`는 비대화형이면 파일 실재로 대체하고 같은 세션 생성분은 대기 메모, 글로벌 한정자 `머신=<이름>`(스킬 별칭과 구분), 개수 표기 제거, 머지 동률은 00 STEP 5 참조, §D-2 팁의 유지보수자 전용 백업 문장 삭제 · §L에 세션 중 CLAUDE.md 로드 🟡 |
> | v1.84 | 2026-09-17 | **최적화**: R20 적용 — 구 행 v1.52~v1.69를 축 요약 행에 병합(버전 행 15개, 규칙 불변) |
...
- ⓐ 글로벌 행동 규칙(§D-2 CLAUDE.md) ⓑ 글로벌 보안(§D-3 deny·hooks) ⓒ `/resume`·`/wrap`·`dropin-` 계열 스킬(재적용 `dropin-apply`·점검 `dropin-check`·문서 최신화 `dropin-update`)(§D-4·F-2 — 이름에 **별칭**을 붙일지는 00 STEP 3가 묻는다. 단독 적용이면 그 자리에서 묻고 기본은 `별칭=없음`) ⓓ 프로젝트 기록 체계(§E·F-1: docs/·CLAUDE.md·.gitattributes·§F-5 .gitignore) ⓔ 프로젝트 권한(§F-3 settings.json) ⓕ effort 묶음·캐시 가이드(§D-6 `modelSettings` 모델별 저장값을 `~/.claude/settings.json`에 기록 + §D-7 캐시 습관은 안내)
- **00의 모드별 기본값**(00 STEP 3에서 왔을 때 — 사용자 모드가 아니면 이 매핑으로 자동 확정하고 질문 생략): **최소 = ⓑⓒⓓ**(보안+스킬+기록 체계 — ⓒ를 빼면 §F-1 블록이 전제하는 `/resume`·`/wrap`이 없고 §D-5의 세션 스킬 자동완성 검증도 통과 불가라, "기반"이 성립하지 않는다) / **권장·전체 = ⓐ~ⓕ 전부**. 어느 쪽이든 ⓑ는 빼지 않는다. **이 매핑은 대상에 프로젝트가 있다고 전제한다** — 대상이 글로벌(PC)뿐이면 아래 줄의 ⓓⓔ 제외가 **이 매핑보다 우선**해 최소 모드도 ⓑⓒ만 남는다.
...
- **선택 해제·제거 절차** (4단 규약의 01 몫 — ⓐⓑⓕ는 **기존 사용자 파일에 블록·키를 얹는** 산출물이라 되돌림이 파일 삭제가 아니다): ⓐ `~/.claude/CLAUDE.md`의 §D-2 블록만 제거(파일은 남긴다) ⓑ `~/.claude/settings.json`에서 §D-3이 넣은 deny·hooks **배열 항목만** 제거(키·파일은 남긴다 — 사용자가 원래 가진 deny 항목까지 지우면 보안이 후퇴한다) ⓒ `~/.claude/skills/{resume,wrap,dropin-apply,dropin-check,dropin-update}/`(별칭을 붙였으면 그 이름) 폴더 삭제(B형이 프로젝트 `.claude/skills/`에 커밋한 사본이면 그 경로) ⓓ §E·F-1이 **만든 파일만** 제거 — `docs/`의 PROJECT_PLAN·PROGRESS·DECISIONS(단위·archive 경로 포함)는 **내용이 쌓였으면 보존 여부를 묻고 기본 보존**(기록은 설치물과 함께 되돌리지 않는다 — 다인 repo면 팀 기록이다), `docs/` 폴더는 비었을 때만 삭제(다른 문서의 산출물·사용자 문서가 같은 폴더에 산다) + `CLAUDE.md`의 §F-1 SSOT 블록·`.gitattributes`의 §F-1 줄·`.gitignore`의 §F-5 줄만 제거(파일은 남긴다) ⓔ 프로젝트 `.claude/settings.json`에서 §F-3이 넣은 권한 항목만 제거(파일은 남긴다) ⓕ `~/.claude/settings.json`의 §D-6 `modelSettings` 키만 제거(파일은 남긴다). **주의**: ⓐⓑⓒⓕ는 PC 전역이라 다른 프로젝트·다른 드롭인 항목도 같이 쓴다 — 지우기 전에 적용 기록(`dropin-applied`)으로 그 블록·키·폴더가 우리 산출물인지, 같은 PC의 다른 프로젝트 기록이 쓰는지 먼저 확인하고, 아니면(또는 다른 프로젝트가 쓰면) 지우지 않는다. 제거 후 기록은 00 STEP 3 질문 ②대로 전환한다(항목만이면 `01 vX.Y(<항목> 미선택 — 사용자 해제)`, 전부면 `미선택: 01(사용자 해제)`).

**STEP 4 — 확인.** 무엇을 만들었는지 요약 보고 → **§검증** → 승인 후 커밋 안내(§E 커밋 규칙).
**§검증** — 두 축을 **둘 다** 증거로 낸다(대상이 **글로벌 전용**이면 ⓓⓔ를 안 만드니 ②는 `해당 없음`을 명시해 갈음하고, ①의 ⓐ·ⓑ 실측이 필수 증거다 — 위 "글로벌(PC)뿐이면" 규칙과 짝). ① **글로벌**: `/` 자동완성으로 `/resume`·`/wrap`·`/dropin-apply`·`/dropin-check`·`/dropin-update`(§D-5 — 자동완성을 볼 수 없는 비대화형 실행이면 §D-4 스킬 실물 파일 존재로 대체). ⓐ를 적용했으면 `~/.claude/CLAUDE.md` **실재 + `/context`의 Memory files 목록에 실제로 로드됐는지**(§D-5 — 파일 실재만 보면 위치·제외 설정 때문에 안 읽히는 경우를 놓친다. `/context`는 사용자 명령이라 **비대화형이면 파일 실재로 대체**하고, 이번 세션에서 만든 파일이라 목록에 없으면 실패로 단정하지 말고 `새 세션에서 /context 재확인` 대기 메모로 남긴다), **ⓑ는 글로벌 `settings.json`의 `deny` 시크릿 차단(+hooks를 넣었으면 `hooks` 키)을 실측**한다 — "빼지 않는다"로 필수화한 항목인데 이 확인이 없으면 프로젝트 deny만 보게 된다. §F-2 **B·C를 골랐으면 스킬 본문이 해당 변형인지 내용으로 확인**(예: B의 단위 판별 서술) — 실물 존재만 보면 A가 그대로 남아 있어도 통과한다. **ⓕ를 골랐으면 `~/.claude/settings.json`의 `modelSettings`에 §D-6 설치 때 확인한 가용 모델 ID 키가 들어 있는지 실측**한다(예시 블록의 키가 아니다 — 키·값을 달리 정했으면 그 사실을 기록에 남긴다) — 산출물이 글로벌 파일이라 ①에 속한다. 안내만 하던 항목이 파일을 쓰는 항목이 됐으므로, 실측 없이는 "골랐다"와 "적용됐다"가 갈린다. ② **프로젝트 산출물**: 이번에 만든 것의 **파일 경로를 실제로 조회해** 보고 — ⓓ `docs/PROJECT_PLAN.md`·`PROGRESS.md`·`DECISIONS.md`(단위 분할이면 그 경로)·`CLAUDE.md`의 §F-1 블록·`.gitattributes`(`git check-attr merge docs/PROGRESS.md` = `union`) / ⓔ `.claude/settings.json`(JSON 파싱 성공 + `deny`에 시크릿 차단 존재 + `additionalDirectories`가 있으면 각 경로가 **이 머신에 실재하고 상대 경로**인지 — §F-3 폴백으로 `.claude/settings.local.json`에 절대경로를 넣었으면 **그 파일의 경로도 같은 기준으로 실재 확인**) / §F-5를 적용했으면 `.gitignore`의 개인 파일 패턴, A-1(통합)이면 루트 라우터 `CLAUDE.md`(§F-2 D) 실재도 함께. **①만 보면 검증이 통과한다** — 그 넷은 글로벌 스킬이라 이 프로젝트에 아무것도 안 만들어도 뜬다(신규 설치에서 `docs/`가 통째로 빠져도 "검증 OK"가 나온다). **적용 기록**: 대상 `CLAUDE.md` 맨 아래 `<!-- dropin-applied: … -->` 한 줄에 이 문서의 **현재 버전**(최상단 "문서 버전" 표기에서 읽음)을 `01 vX.Y` 형식으로 추가/갱신한다(형식·재적용 규칙은 00 §A — 00 없이 단독 적용해도 남긴다). **글로벌만 적용(대상이 PC뿐)이면 `~/.claude/CLAUDE.md`에 `<!-- dropin-applied(글로벌 · 머신=<이름>): … -->`처럼 스코프·머신 한정자를 붙인다**(00 §A STEP 5 형식과 동일 — 글로벌은 PC당 하나라 밝히지 않으면 백업 미러에서 여러 PC 기록이 서로를 덮는다. `<이름>`에는 OS 호스트명을 그대로 쓰지 않는다 — 스킬 `별칭=`과 다른 축이다).
...
| **🅰️ 통합 A-1** | 여러 repo가 **한 상위 폴더 아래** 나란히 | §E-2 (+ 선택 §E-4 루트 레이어 — **기본 아니오**) |
| **🅰️ 통합 A-2** | 여러 repo가 **흩어져** 있음 | §E-2 (+ 선택 §E-4 — **기본 아니오**. 단 `--add-dir` 필요) |
| **➕ MSA 단위분할** | 위에 더해 **동시성 ⓒ**(여러 명·같은 갈래) 또는 ⓑ에 갈래 셋 이상 | §E-3 를 각 repo에 추가 적용 |
| **혼합형** | 상위 폴더 아래에 **독립 repo와 비-git 폴더가 섞여** 있음 | 통합 레이어(§E-4)는 그대로 두고, **단위별로 git 여부를 따로 판정**한다 — 비-git 단위엔 아래 면제를 개별 적용하고 커밋은 상위 repo에서 수행. `/resume`·`/wrap`의 "루트+하위 각각 git status"도 git 단위에만 돌린다. **비-git 단위의 기록 위치는 우선순위로 정한다** — ① 그 단위 안에 자체 `docs/`가 있으면 거기(면제를 그 단위에 개별 적용) ② 없으면 상위 repo의 `docs/<단위>/`(§F-2 C 판별) — **둘 다 만들지 않는다**(두 곳에 생기면 어느 쪽도 정본이 아니게 된다) |
...
> 아래 명령 블록은 Windows(PowerShell) 기준이다 — macOS·Linux는 홈을 `~`로 읽고 D-1을 `mkdir -p ~/.claude/skills/{resume,wrap,dropin-apply,dropin-check,dropin-update}`로 대체한다(만드는 내용물 D-2~D-4는 OS 무관. OS별 CLI 설치 분기는 04 §2와 같은 방식).
...
New-Item -ItemType Directory -Path .claude/skills/dropin-apply -Force
...
```
> 위 폴더들이다(개수는 세지 않는다 — R14, 늘어나면 이 문장 대신 위 코드블록이 정본이다) — §D-4가 `dropin-` 계열을 복사해 넣고 §D-5가 **위 폴더 전부**의 자동완성을 성공 기준으로 삼는다.
...
> 🔴 **아래 블록이 유일한 정본이다 — 이것만으로 완전한 글로벌 `CLAUDE.md`가 나온다.** 어느 저장소를 가졌든 **설치 결과는 같아야 하므로**(개발 저장소로 깔든 배포 저장소로 깔든 동일한 최신 구성), 설치는 **문서 블록만** 보고 한다. **`global-config/` 미러를 설치 소스로 쓰지 않는다**(예외: §D-4의 `global-config/skills/dropin-apply`·`dropin-check`·`dropin-update`는 배포되는 설치 자산이다 — 목록의 정본은 배포 워크플로의 복사 대상이다) — 그것은 유지보수자가 자기 PC의 `~/.claude`를 되살리려고 두는 **사설 백업**이고 개발 저장소(`claude-dev`, 비공개)에만 있어, 미러를 읽는 순간 **가진 저장소에 따라 결과가 갈린다**(가진 사람은 개인 스킬·머신 키까지 얹힌 구성을, 없는 사람은 골격을 받는다). 미러를 "정본"으로 가리키던 옛 문언은 배포본 설치자를 **없는 파일로 보내 골격조차 못 깔게 했다**(2026-09-10 실제 신고). 이 블록에 부족한 것이 있으면 **그것은 이 블록의 결함**이니 여기를 고친다 — 사용자가 다른 데서 구해 오게 하지 않는다. **`dropin-applied` 줄은 00 STEP 5가 이 PC 기준으로 새로 쓴다** — 다른 PC의 줄을 옮기면 하지도 않은 설치를 주장하고 `/dropin-check`가 그것을 1차 근거로 믿는다.
```markdown
# CLAUDE.md (Global)
...
## 5. Output Style — 간결하게, 전체 파일 재출력 금지(변경분만), 간결은 분량에만(경어가 있는 언어면 정중체 유지).
...
```json
{
  ...
  "hooks": {
    ...
    "SessionStart": [ { "matcher": "compact", "hooks": [
      ...
      { "matcher": "clear", "hooks": [
      { "type": "command", "command": "echo 'clear 후 새 컨텍스트: /resume으로 재개하라. 직전 세션에 미기록 결정이 있었다면 먼저 /wrap을 안내하라.'" } ] } ]
  }
}
```
...
**선택 확장 — `PreCompact` `manual` 게이트**(Windows 전용 — `"shell": "powershell"`. macOS·Linux엔 병합하지 않는다: 실행할 수 없는 셸을 부른다). `/compact`를 직접 실행할 때 `/wrap` 흔적(`docs/**/PROGRESS.md`가 미커밋 변경이나 직전 커밋에 있음 — 비-git 폴더면 오늘 수정)이 없으면 exit 2로 막고, 5분 안에 다시 실행하면 통과시킨다. 쓰려면 위 블록 `hooks.PreCompact` 배열에 이 항목을 더한다:
  ...
  "hooks": [
    {
      ...
      "shell": "powershell",
      "timeout": 15,
      "statusMessage": "wrap-gate: /wrap 선행 확인 중",
      "command": "$d=@(Get-ChildItem docs -Recurse -Filter PROGRESS.md -EA SilentlyContinue); if($d.Count -eq 0){ Write-Output '[wrap-gate] 이 폴더에서 기록 파일(docs/**/PROGRESS.md)을 찾지 못함 - 참고: /wrap > /clear > /resume 권장'; exit 0 }; $k=((Get-Location).Path.ToLower() -replace '[^a-z0-9]','_'); if($k.Length -gt 80){ $k=$k.Substring($k.Length-80) }; $m=Join-Path $env:TEMP ('claude-compact-override-'+$k); if((Test-Path $m) -and (((Get-Date)-(Get-Item $m).LastWriteTime).TotalMinutes -lt 5)){ [System.IO.File]::Delete($m); Write-Output '[wrap-gate] override 확인 - 압축 진행'; exit 0 }; if(Test-Path $m){ [System.IO.File]::Delete($m) }; $git=[bool](git rev-parse --is-inside-work-tree 2>$null); if($git){ $ok=([bool](@(git status --porcelain 2>$null) -match 'PROGRESS\\.md')) -or ([bool](@(git diff-tree --no-commit-id --name-only -r HEAD 2>$null) -match 'PROGRESS\\.md')); $why='PROGRESS.md가 직전 커밋에도 미커밋 변경에도 없음' } else { $ok=[bool](@($d | Where-Object { $_.LastWriteTime.Date -eq (Get-Date).Date })); $why='비-git 폴더 - PROGRESS.md가 오늘 수정되지 않음' }; if($ok){ Write-Output '[wrap-gate] /wrap 흔적 확인(PROGRESS.md 변경) - 압축 진행. 참고: /clear > /resume이 요약 왜곡 없이 더 안전하다'; exit 0 }; New-Item -ItemType File -Path $m -Force | Out-Null; [Console]::Error.WriteLine('[wrap-gate] /compact 차단 - /wrap 흔적 없음('+$why+'). 권장: /wrap > /clear > /resume. 그래도 압축하려면 5분 내 /compact 재실행'); exit 2"
    }
  ]
}
```

### D-4. 기본 `/resume`·`/wrap` + `dropin-` 계열 스킬(재적용·점검·최신화) (단일 repo용 기본형) — §F-2 A 참조. (통합/MSA는 §F-2 B·C — 동명 스킬은 **개인(글로벌)이 프로젝트를 이기므로** 글로벌 쪽을 교체한다, §F-2 주의)
- **`/dropin-apply`**(구성 재적용 — 문서·스킬 실물·설정을 00 절차로 한 번에. 자연어 "이 문서를 학습해서 적용해줘"와 같은 일을 하는 슬래시 진입점이라, 진입점이 자연어뿐이면 존재를 모르는 사람에게는 없는 것과 같다)·**`/dropin-check`**(적용 상태 점검, 읽기 전용)·**`/dropin-update`**(문서 사본 최신화, 승인 후 교체)는 환경 무관 **글로벌 스킬** — 문서 저장소의 `global-config/skills/<이름>/SKILL.md`를 `~/.claude/skills/<이름>/`로 복사한다(로컬 클론 없으면 raw: `https://raw.githubusercontent.com/EleninJayTech/claude/main/global-config/skills/dropin-check/SKILL.md` · 같은 경로의 `dropin-apply`·`dropin-update`). 설치 전 같은 이름 구형 `commands/`가 있으면 제거(§F-2 주의). 이 스킬들은 **설치 유형 무관 동작 보장** — 프로젝트 사본 / 로컬 클론 참조 / raw 직접(사본·클론을 나중에 지워도 계속 동작), 판별 근거는 00 STEP 5 기록의 `출처=` 필드(그 필드가 도입된 이후 기록 — 없으면 각 스킬이 사본→클론→raw 순으로 판별한다). **별칭은 받는 쪽 폴더명에만 붙인다** — 복사 **원본** 경로(`global-config/skills/dropin-check/…`·위 raw URL)는 문서 저장소 쪽이라 **바뀌지 않는다**. 원본까지 별칭으로 바꿔 부르면 raw가 404다.
...
- **절약 프로필** (Pro 요금제·한도 관리 사용자 — 00 STEP 3에서 선택): 절약은 품질 유지 전제의 2순위이므로 **모델 배정 하향 + 낭비 제거**로만 아낀다(effort는 프로필로 갈리지 않는다 — 아래).
  - 위 묶음이 이미 하위 모델을 `medium`으로 두므로 **절약 프로필이 따로 더 내릴 자리는 상위 모델뿐인데, 거기서 아끼면 재작업이 더 비싸다**(아래 불변). 절약의 실제 레버는 effort가 아니라 **모델 열**(02 §F 절약 배정 — 일상도 Sonnet, 고난이도는 Sonnet 메인 + advisor)과 아래 컨텍스트 습관이다. 묶음을 안 쓰면 세션을 `/effort medium`으로 시작하고 어려운 작업에 들어갈 때만 `/effort high`↑(슬라이더 `s`로 세션 한정 — Enter는 저장값을 바꾼다). `ultracode`·`max`는 비권장(토큰 소모 큼).
...
- ⚠️ **함정 — 프로젝트 값이 `/effort` 선택을 조용히 덮어쓴다**: `/effort`로 고른 값은 **User 스코프**에 저장되는데, 그 프로젝트 `.claude/settings.json`에 `effortLevel`이 있으면 Project가 이겨서 **새 세션마다 프로젝트 값으로 되돌아간다**. 경고 메시지는 없다. "왜 자꾸 xhigh로 시작하지?" 싶으면 `.claude/settings.json`부터 확인할 것.
...
- `/model`(모델 선택)과 짝 명령: 모델에 따라 사용 가능한 단계가 다르다(위 모델별 지원 참고). 모델을 바꾸면 **그 모델의 저장값**(위 `modelSettings` 묶음, 없으면 모델 기본값)이 적용된다 — 직전 모델에서 고른 값이 따라오지 않는다.
...
### E-1. 🅱️ 단일 repo (B)
- `docs/` **flat**: `PROJECT_PLAN.md` · `PROGRESS.md` · `DECISIONS.md` (§F-6 양식). 계획서를 쓰는 repo는 `docs/plans/`(단위 분할이면 `docs/<단위>/plans/`) — 02 ⓔ 산출물, 없어도 된다.
- 스킬: 글로벌 기본형(§F-2 A) 사용. `.gitattributes` merge=union(§F-4)은 **동시성 ⓑ·ⓒ면 필수**(§I 2).
- 커밋: `git add CLAUDE.md docs .claude .gitignore .gitattributes` → 한 repo에서 커밋.
...
- **팀원마다 설치 층위가 달라도 공존한다** — 어떤 팀원은 루트(이 레이어)에서, 어떤 팀원은 하위 repo 하나(§E-1)에서 적용해도 충돌 지점이 없다: 루트 레이어는 하위 repo를 손대지 않고, 하위 적용은 그 repo 안에만 산출물을 만든다. 단 **상위 폴더 자체가 팀 공유 git repo면** 루트 라우터·INDEX가 커밋되어 하위 repo만 쓰는 팀원에게도 도착하므로, 커밋 전에 팀 합의를 받는다(**소유는 층과 별개 축이다** — 개인 소유면 비공유·자유, 팀 공유 repo면 합의·커밋 대상. 아래 D·E 템플릿의 "개인" 표기는 개인 소유일 때의 값이고, 팀 소유면 "팀 공유"로 바꿔 쓴다).
...
> **언어**: 아래 블록과 이 블록이 규정하는 산출물(`CLAUDE.md` 본문·`docs/` 기록 항목·커밋 메시지)은 00 STEP 3의 **`언어=ko|en`** 값으로 쓴다(00 없이 단독 적용이면 그 자리에서 묻는다). 형식·키워드(`[갈래][상태]`·`Done`/`Pending`·`dropin-applied` 줄의 필드와 상태 어휘(`미선택:`·`대기`·`게이트 차단`·`해당 없음` 등 — 00 STEP 5 표)·PROJECT_PLAN의 `미해결/관찰 중`·`다음 ○○: YYYY-MM경`)는 **언어와 무관하게 그대로** 둔다 — `/resume`·`/wrap`·`check.ps1`이 그 문자열로 찾는다.
...
- **공유 기록·커밋 메시지의 말투는 평서체(-다/-했다)로 통일**한다 — 대화 말투(글로벌 §5)와는 별개 축이다. 다인 repo에서 각자의 글로벌 설정이 제각각이면 **한 파일 안에서** 말투가 갈리는데, 개인 취향이 아니라 공유물 오염이라 이쪽(git 전파) 규칙이다. 인용(사용자 발언·훅 문자열·문서 원문)은 예외.

### 동시편집 충돌
- PROGRESS·DECISIONS는 append 전용(기존 줄 수정 금지 — 예외는 아래 되돌림·Superseded **표시** 한 조각뿐이고, 병합 충돌이 나면 표시 있는 줄을 남긴다). .gitattributes의 merge=union이 동시 append를 자동 합침.
- **PROJECT_PLAN.md·CLAUDE.md엔 merge=union을 걸지 않는다** — 체크박스 토글·기록 줄 교체는 줄 수정이라 union이면 양쪽이 다 남아 파손된다. 충돌은 손으로: PROJECT_PLAN은 체크박스=완료 우선·미해결=합집합, CLAUDE.md의 dropin-applied는 적용일 최신 줄 기준(동률·`출처=` 처리는 00 STEP 5).
...
### 세션 워크플로 규율
- **같은 폴더에서 두 세션을 동시에 돌리지 않는다** — 작업 트리·git 인덱스·기록 파일을 공유해 서로의 미커밋이 섞인다(`merge=union`은 git 병합 때만 동작해 여기선 무력). 두 번째 작업은 `claude --worktree <이름>`으로 열거나, 이어받을 대화는 `claude -n <이름>`·`/rename`으로 이름을 붙여 `claude --resume <이름>`으로 연다 — `claude -c`는 그 폴더의 가장 최근 대화 하나만 연다. 아래 /resume·/wrap의 "세션 밖 변경" 처리는 이 습관이 깨졌을 때의 안전망이다.
...
### 길이 관리
- PROGRESS·DECISIONS가 약 800줄 **또는 약 120KB**(먼저 닿는 쪽)를 넘으면 가장 오래된 항목부터 docs/archive/로 옮기고 활성 파일 **최상단(제목 바로 아래) 포인터 한 줄**("이전 기록: docs/archive/…"). 두 축인 이유 — 1항목=1줄로 적는 저장소는 줄 수가 임계의 4분의 1일 때 이미 100KB를 넘는다. **얼마나 자를지는 달력이 아니라 목표치로 정한다** — 활성 파일이 **임계의 절반 이하**가 될 때까지(가능하면 날짜 경계에 맞춰). 분기·월 같은 달력 단위는 부피와 무관해서, 그 단위로 잘라도 임계 아래로 못 내려가면 매 세션 다시 발화한다. 맨 아래가 아니라 최상단인 이유 — /resume가 최근 ~5항목만 읽어 아래 포인터는 보이지 않는다. 이후 append는 **포인터 줄 아래부터**(포인터 최상단 고정). **아카이브 커밋은 같은 세션에 즉시 푸시한다** — `merge=union`은 삭제를 지키지 못해, 그 사이 다른 PC의 append와 병합되면 옮긴 항목이 되살아난다(push 권한이 없으면 PR을 즉시 연다). **과거 이력 검색은 활성 파일이 아니라 `docs/` 폴더 단위로**(아카이브 자동 포함) — 파일만 검색하면 "기록 없음"으로 오판한다. **아카이브 커밋은 같은 세션에 즉시 푸시한다** — 이 파일들은 `merge=union`이라 동시 append는 살리지만 **삭제는 지키지 못한다**. 다른 PC가 그 사이 append하고 병합되면 옮긴 항목이 활성 파일로 되살아나 id가 중복되고 크기 축이 원복된다. 노출 시간을 0에 가깝게 줄이는 것이 유일한 방어다.
- `PROJECT_PLAN`의 "미해결/관찰 중"은 **종결 항목이 40건**을 넘으면 미해결 아카이브 — 가장 오래된 종결일부터 `docs/archive/`로 옮겨 **20건 이하**로 하고, **열린 항목은 옮기지 않는다**(옮기면 /resume이 인계 항목을 못 본다). 그 절 머리에 포인터 한 줄. 크기가 아니라 건수 축인 이유 — 절이 여럿이라 파일 크기는 어느 절 탓인지 못 가리킨다.
...
> **별칭 치환 규칙**(00 STEP 3 `별칭=`): 치환 대상은 **`/<원래이름>` 토큰**뿐이다 — 슬래시 붙은 호출 형태만 바꾸고, `${CLAUDE_SKILL_DIR}` 같은 런타임 변수·파일 경로·설명 문장 속 낱말은 건드리지 않는다. **기록된 `별칭=`이 적용된(또는 이번에 적용할) 스킬을 가리키는 토큰만** 치환한다 — 회차와 무관하다(재적용에서 한 스킬만 다시 깔아도 그 본문이 부르는 다른 별칭 스킬은 치환 대상이다). 별칭을 안 붙인 스킬을 가리키는 참조를 함께 바꾸면 **없는 명령을 가리킨다**. ⚠️ **상호 참조는 편도라 놓치기 쉽다**: 아래 A·B·C 모두 **wrap 본문이 `/resume`을 언급**하지만 resume 본문은 `/wrap`을 언급하지 않는다(대신 `/dropin-check`·`/dropin-update`를 언급한다) — "서로 참조하니 양쪽만 보면 된다"고 넘기면 wrap 쪽의 언급이 그대로 남는다(**개수는 세지 않는다** — R14, 템플릿이 늘 때마다 숫자가 어긋난다). 이 규칙은 **§F-1 블록과 §D-3 hooks 문구**(SessionStart·PreCompact의 echo 문자열 — 선택 확장 포함)**에도 그대로 적용**된다(그 블록·템플릿·문구에 있는 `/resume`·`/wrap` 토큰 **전부**가 대상이고, §F-1 블록은 각 프로젝트 `CLAUDE.md`에 복붙된다). 별칭을 바꾸면(00 STEP 5 rename) 이 PC의 스킬 본문·hooks 문구·글로벌 §9를 함께 갱신한다 — 다른 프로젝트의 §F-1 블록은 커밋물이라 고지만 한다.
>
...
> ⚠️ **이 포함 관계는 §F-1 규약을 공유하는 세 변종 사이에서만 성립한다.** 설치 전에 **이 PC로 열게 될 repo에 동명 `resume`·`wrap`이 있는지** 본다(`ls <각 repo>/.claude/skills/{resume,wrap}`) — 있으면 그것이 §F-1을 따르는지 확인한다(첫 화면에 아카이브 포인터 `최상단(제목 바로 아래)`·`[갈래][상태]` 양식이 있는가). **외부에서 독립 저작된 동명 스킬이 하나라도 있으면 글로벌에 두지 않는다** — `personal > project`라 그 repo에서도 이쪽이 실행되는데, C형은 폴백이 촘촘해 **에러 없이 어긋난 채 성공**한다(침묵형 실패). 그 경우 처방은 repo마다 프로젝트 사본(A/B/C 중 그 repo에 맞는 것)을 넣고 글로벌은 비우는 것이다. `dropin-` 계열은 남의 규약과 겹칠 일이 없어 대상 아니다.
...
>
> 🔴 **이 우선순위를 다른 스코프에 옮겨 적지 마라 — 방향이 하위 시스템마다 다르다.** **설정**은 좁은 쪽이 이기고(관리형 > 명령행 > 프로젝트 로컬 > 프로젝트 공유 > 사용자), **스킬**은 넓은 쪽이 이기며(위), **서브에이전트**는 **가까운 쪽**이 이긴다(프로젝트 > 개인 — 작업 디렉터리에 가장 가까운 정의). 같은 "스코프"라는 낱말이 세 방향으로 작동한다. 한쪽 직관을 그대로 옮겨 쓴 문장이 실제로 여러 회차의 감사를 **통과했다**(2026-08-31 정정, 00 STEP 2·02 STEP 4) — 문서끼리는 모순되지 않고 바깥과만 어긋나 있어 내부 검사로는 잡히지 않는다. 스코프를 말할 때마다 **그 시스템의 공식 문서로 방향을 다시 확인한다.**
...
> ⚠️ **동명 스킬이 `/` 메뉴에 2개 뜨는 것은 B·C형에서 정상이다**: 위 우선순위 때문에 B·C는 repo `.claude/skills/`에 커밋한 사본과 교체한 글로벌 사본이 **한 PC에 공존**하고, 자동완성은 출처별로 따로 등록하므로 `/resume`·`/wrap`이 같은 이름으로 2개 보인다(공식 2026-08-23 확인: 메뉴 병합·중복 경고 규정 없음, 실행은 개인 쪽. 루트+하위 repo 양쪽 `.claude/skills/`도 같은 이유로 둘 다 뜬다 — 프로젝트 스킬은 상위 폴더까지 로드). 점검 포인트는 **개수가 아니라 두 사본의 내용 일치** — `git diff --no-index ~/.claude/skills/<이름>/SKILL.md .claude/skills/<이름>/SKILL.md`가 비어야 하고, 다르면 "한 이름 한 정의" 위반이니 글로벌을 B·C 내용으로 다시 교체한다. 구형 `commands/` 잔재로 인한 2개는 이와 달리 삭제 대상이다(위 "중복 등록 방지").
...
2) docs/<단위>/PROGRESS.md 최상단(최근 ~5항목(상단 날짜가 역순이면 **날짜 기준**)만 — 길면 항목당 앞 ~700자, 통째 읽기 금지. 다인 공유면 현재 브랜치·PWD가 가리키는 **갈래를 우선** 읽고 남의 갈래는 한 줄 요약) + docs/PROJECT_PLAN.md + 최근 DEC 3건 → "이 단위 지난 X, 다음 Y?" 보고(`[Pending]`·`[Blocked]`는 PROJECT_PLAN "미해결/관찰 중"**과 대조**해 거기 없으면 닫힌 것 · 항목이 **다음 행동 없음**을 명시하면 Y를 재구성하지 말고 새 작업을 묻는다).
...
append 후 해당 PROGRESS·DECISIONS 부피 확인 — ~800줄 또는 ~120KB 초과면 아카이브 안내(가장 오래된 항목부터 임계의 절반 이하까지). PROJECT_PLAN "미해결/관찰 중"의 종결 항목이 40건을 넘으면 미해결 아카이브 안내(가장 오래된 종결일부터 20건 이하로 · 열린 항목은 옮기지 않는다).
**(유지보수자 전용 단계 — remote가 `EleninJayTech/claude-dev`인 개발 저장소 클론이 있을 때만. 배포 저장소(`claude`) 클론의 `global-config/`엔 배포용 `skills/dropin-*` 계열뿐이라(목록의 정본은 배포 워크플로의 복사 대상 — 개수를 적지 않는다) 미러가 아니다 — 거기서는 조용히 생략하고 결함으로 보고하지 않는다. 유지보수자 PC인데 개발 클론을 찾지 못했으면 생략 대신 "미러 대조 미확인" 한 줄)** 이번 세션에 `~/.claude/`나 그 저장소 `global-config/`를 고쳤으면 화이트리스트(정본은 개발 저장소 `global-config/README.md` 표) 해시 대조 — 차이 시 방향 판정+복사 안내. **판정 기준은 A와 동일**(구성 성격 4키만 실제 차이·포함 기준이 정본·글로벌 `CLAUDE.md`의 `dropin-applied` 줄 제외·줄바꿈 정규화 후 비교).
```
...
2) <repo>/docs[/<단위>]/PROGRESS.md 최상단(최근 ~5항목(상단 날짜가 역순이면 **날짜 기준**)만 — 길면 항목당 앞 ~700자, 통째 읽기 금지. 다인 공유면 현재 브랜치·PWD가 가리키는 **갈래를 우선** 읽고 남의 갈래는 한 줄 요약) + 최근 DEC 3건 + <repo>/docs/PROJECT_PLAN.md + 루트 docs/INDEX.md 최근 항목 → 보고(`[Pending]`·`[Blocked]`는 PROJECT_PLAN "미해결/관찰 중"**과 대조**해 거기 없으면 닫힌 것 · 항목이 **다음 행동 없음**을 명시하면 다음 행동을 재구성하지 말고 새 작업을 묻는다).
...
5) repo별 변경 파일 보고(**이 세션이 편집한 파일** 기준 — 그 밖의 미커밋은 **세션 밖 변경**으로 따로 표시, A와 동일) + 미커밋 경고. 커밋은 repo별 따로(팀 양식). append한 PROGRESS·DECISIONS가 ~800줄 또는 ~120KB 초과면 아카이브 안내(가장 오래된 항목부터 임계의 절반 이하까지). PROJECT_PLAN "미해결/관찰 중"의 종결 항목이 40건을 넘으면 미해결 아카이브 안내(가장 오래된 종결일부터 20건 이하로 · 열린 항목은 옮기지 않는다).
...
- 커밋은 repo별로 따로(독립 git). 루트 레이어는 소유에 따라 비공유(개인) 또는 팀 repo 커밋(팀 — §E-4).
```

**E) 루트 `docs/INDEX.md`** (얇은 크로스-repo 인덱스)
```markdown
# 통합 작업 인덱스 (개인 또는 팀 공유 · 얇게 · 링크 기반)
...
> ⚠️ **`additionalDirectories`는 크로스-repo 작업이 있을 때만 — 단일 repo면 이 키를 통째로 뺀다.** 위 블록의 `../<자주 함께 고치는 repo>`는 플레이스홀더라, 지우지 않고 그대로 적용하면 **존재하지 않는 경로가 팀 공유 파일에 커밋된다**(2026-08-13 신규 설치 실행에서 발견). JSON엔 주석을 달 수 없어 블록 안에 조건을 적을 수 없으니 이 줄이 정본이다. 경로는 **상대 경로로만** 적는다 — 절대경로는 그 PC에서만 유효해 팀 공유 파일에 커밋되면 다른 팀원 PC에서 부재 경로가 된다(머신 종속 경로가 꼭 필요하면 개인 파일 `.claude/settings.local.json`으로).
...
.claude/sessions/
.claude/logs/
# claude --worktree·에이전트 isolation이 만드는 격리 checkout (주석은 별도 줄 — .gitignore는 줄 머리 #만 주석이다)
.claude/worktrees/
# Claude 개인 구성 (커밋 금지)
...
> ⚠️ **같은 폴더에서 두 세션을 동시에 돌리지 않는다**(§F-1에 같은 줄이 있어 프로젝트 `CLAUDE.md`에도 복붙된다) — 작업 트리·git 인덱스·기록 파일을 공유해 서로의 미커밋이 섞인다(`merge=union`은 git 병합 때만 동작해 여기선 무력). 두 번째 작업은 `claude --worktree <이름>`(격리 한계는 02 §C)으로 열고, 이어받을 대화는 `claude -n <이름>`·`/rename`으로 이름을 붙여 `claude --resume <이름>`으로 연다 — `claude -c`는 그 폴더의 **가장 최근 대화 하나**만 연다.
...
| 동시성 ⓐ/ⓑ/ⓒ(§I 2) | **ⓑ부터 merge=union 필수**, ⓒ면 §E-3 단위분할+솔루션aware(ⓑ도 갈래 3+면 권장), ⓐ면 flat |
...
- **샌드박스** 🔴: `/sandbox` 또는 `sandbox.enabled`로 켜는 OS 수준 격리(Bash 명령+자식 프로세스의 파일·네트워크 접근을 OS가 강제). 지원: **macOS(Seatbelt) · Linux · WSL2(bubblewrap+socat)**. **네이티브 Windows는 여전히 미지원** → Windows에선 WSL2에서 돌리거나, allow 좁게 + deny 규칙 기반으로 운용.
  - 샌드박스 기본 읽기 정책은 **컴퓨터 전체 읽기 허용**(일부 시스템 경로 제외)이라 `~/.ssh`·`~/.aws`는 **`sandbox.credentials`**(v2.1.187+)로 명시 차단하거나 env 토큰은 `mode: "mask"`(v2.1.199+)로 대체 — 형식은 `credentials.files[]`={path,mode}·`credentials.envVars[]`={name,mode}(mode=`deny`|`mask`)이고, **`mask`는 user/managed 스코프에서만 유효하며 `network.tlsTerminate`가 필요**하다. v2.1.224+로 확장 옵션 8종(`extract`·`decode`(JWT)·`maskClaims`·`maskDuplicates`·`onExtractNoMatch`·`injectHosts`·`awsPairs`·`sigv4`)이 추가됐다 — 세부 값별 매칭·마스킹 규칙을 더 촘촘히 지정할 때 쓴다. 전 서브프로세스에서 Anthropic·클라우드 자격증명 제거는 `CLAUDE_CODE_SUBPROCESS_ENV_SCRUB`. 파일 격리만 끄고 네트워크 격리는 유지하려면 `sandbox.filesystem.disabled`(v2.1.216+, user/managed 스코프만 유효).
...
**문서 정보** — 통합 마스터(범용) **v1.90**. 변경 이력은 최상단 버전 표 참조(유래: 8개 소스 통합 초판 — 최상단 병합 행).
