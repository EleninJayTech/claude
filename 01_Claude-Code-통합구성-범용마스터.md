...
> **문서 버전: v1.47** · 최종 갱신: **2026-08-20** · **최근 재검증: 2026-08-20** · 기준: Claude Code v2.1.237 (Opus 5 · Sonnet 5 · Fable 5)
>
> | 버전 | 날짜 | 변경 내용 |
> | --- | --- | --- |
> | v1.0~v1.32 | 2026-07-03~2026-08-12 | 8개 소스 통합 초판 → 공식 재검증 반복(deny·샌드박스·`attribution`·`effort`) → §D-6 `/effort`·절약 프로필·§0 기본 원칙·문서 번호 재편 → 부분 선택+§J-1 auto mode → **기록 체계**(/resume 원격 최신화 선행·PROGRESS 읽기 상한·아카이브 포인터 최상단·`[Pending]` 승격·정기 항목 `다음 ○○: YYYY-MM경`) → **점검 스킬 배포**(`/dropin-check`·`/dropin-update`, 스코프 우선순위 **enterprise > personal > project** 정정) → /wrap 미러 대조 배선(`출처=`·4키 판정), §F-2 A/B/C 폴백 의무화(비-git·`docs/` 부재·단위 없음), §B 혼합형·루트 라우터 예외. 행별 상세는 저장소 git 이력 |
...
> | v1.47 | 2026-08-20 | **최적화**: R20 적용 — 구 행을 v1.0~v1.32을 축 요약 한 줄로 |
...
**문서 정보** — 통합 마스터(범용) **v1.47**. 변경 이력은 최상단 버전 표 참조(유래: 8개 소스 통합 초판 — v1.0 행).
