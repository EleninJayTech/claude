...
> **문서 버전: v1.82** · 최종 갱신: **2026-09-17** · **최근 재검증: 2026-09-16** · 기준: Claude Code v2.1.273 (Opus 5 · Sonnet 5 · Fable 5.1)
>
> | 버전 | 날짜 | 변경 내용 |
> | --- | --- | --- |
> | v1.0~v1.51 | 2026-07-03~2026-08-21 | 8개 소스 통합 초판 → 공식 재검증 반복(deny·샌드박스·`attribution`·`effort`) → §D-6 `/effort`·절약 프로필·§0 기본 원칙·문서 번호 재편 → 부분 선택+§J-1 auto mode → **기록 체계**(/resume 원격 최신화 선행·PROGRESS 읽기 상한·아카이브 포인터 최상단·`[Pending]` 승격·정기 항목 `다음 ○○: YYYY-MM경`) → **점검 스킬 배포**(`/dropin-check`·`/dropin-update`, 스코프 우선순위 **enterprise > personal > project** 정정) → 점검·최신화 스킬 **설치 유형 전 지원**(`출처=` 판별)·/wrap 미러 대조 배선(4키 판정), 커스텀 /resume·/wrap+`docs/` 기록 체계는 **내장 Tasks 대비 정본 유지**, §F-2 A/B/C 폴백 의무화(비-git·`docs/` 부재·단위 없음), §B 혼합형·루트 라우터 예외. → §A STEP 4 **§검증 신설**(글로벌 스킬+프로젝트 산출물 두 축·글로벌 축 실측·비대화형 대체 증거) → §E-2 하위 적용 단위별 질문·§E-3 단위 도출 기준·§E-4 팀원 층위 혼재 규칙 → §F-3 `additionalDirectories` 상대 경로만 → §D-3 hooks 실물 동기(정본 `global-config/settings.json`·`manual` 게이트) → **/resume 읽기 단위 줄 수→최근 ~5항목**·아카이브 임계 크기 축(~120KB)·예정일 안내 미완료만 → 전면 재검증(deny 우회·샌드박스 Windows 미지원·auto-memory 한도·Read deny→Write 차단·auto mode 기본·`sandbox.credentials`) → §J **격리 3단** → **다인 축 배선**(R21~R24 — 동시성 3분기·갈래·날짜·`[Reverted]`·union 금지)·sfa형 반영(B·C 템플릿·C ⊃ B ⊃ A)·최적화 수회. → 단독 적용 시에도 적용 기록·감지 순서 통일. 행별 상세는 저장소 git 이력 |
> | v1.52 | 2026-08-21 | **/audit 7차(회귀) 반영** — §F-2 **A wrap에 §F-6 양식(갈래·날짜)·되돌림 배선**(대장이 A를 선언해 놓고 비어 있던 자리), 기록 3종 판정을 **경로 기준**으로(`PROJECT_PLAN`=repo `docs/` 루트 · `PROGRESS`·`DECISIONS`=대상 단위 — 단위분할 repo 오보 제거) + A wrap에 단위분할 감지, `[Pending]` **종결자** 배선(PROJECT_PLAN 대조), §B 표·§H를 **동시성 3분기**와 **§E-4 선택**에 동기, 기여자 수는 ⓐ만 가른다, 장수 브랜치면 **날짜 기준** |
> | v1.53 | 2026-08-23 | §D **동명 스킬 메뉴 중복은 B·C형 정상** 경고 신설(글로벌+repo 사본 공존 — 점검은 개수가 아니라 내용 일치, 구형 `commands/` 중복과 구분) |
> | v1.54 | 2026-08-24 | §F-1에 **산출물 언어**(00 STEP 3 `언어=` 값) 한 줄 — 본문은 그 언어로, 형식 키워드는 언어 무관 고정 |
> | v1.55 | 2026-08-25 | §D-2 글로벌 템플릿 §5에 **간결의 적용 범위** 한 줄 — 간결은 분량에만, 경어가 있는 언어면 정중체 유지(설치 후 대화 말투가 반말로 표류하던 부작용) |
> | v1.56 | 2026-08-26 | §F-2 스킬 확장 키에 **인자 키 2종 추가**(`argument-hint:`·`arguments:` 명명 인자 — 공식 문서 확인) |
> | v1.57 | 2026-08-31 | §D-3 deny ⑥에 **경로 앵커의 슬래시 규칙** 추가 — 함정은 슬래시 하나(`/path`)로, 사용자 설정에서는 `~/.claude/` 밑을 가리킨다. `**/`가 작업 디렉터리 기준인 이유와 `//**/`가 필요한 경우를 명시 |
> | v1.58 | 2026-08-31 | §F-2 스킬 우선순위 주의에 **스코프 3방향 대비**(설정=좁은 쪽·스킬=넓은 쪽·서브에이전트=가까운 쪽 — 한쪽 직관을 옮겨 쓴 문장이 여러 회차의 감사를 통과했다), §D-5 확인에 **`/context` Memory files 축**(파일 실재 ≠ 세션 로드) |
...
> | v1.63 | 2026-09-02 | **다음 세션이 이어받을 것은 항목 앞쪽·PROJECT_PLAN에**(§F-1 + §F-2 wrap 템플릿 A·B·C, 대장 R34) — 계획서 경로·다음 행동을 PROGRESS 항목 **끝**에 붙이면 /resume의 700자 컷 밖이라 도달하지 않는다 |
...
**문서 정보** — 통합 마스터(범용) **v1.82**. 변경 이력은 최상단 버전 표 참조(유래: 8개 소스 통합 초판 — 최상단 병합 행).
최종 갱신: 2026-09-17 · 최근 재검증: 2026-09-16 / 참조: Claude Code v2.1.273, Opus 5(v2.1.219+) · Sonnet 5(v2.1.197+) · Fable 5.1(v2.1.257+).
