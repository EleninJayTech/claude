...
> **문서 버전: v1.46** · 최종 갱신: **2026-08-20** · **최근 재검증: 2026-08-20** · 기준: Claude Code v2.1.237 (Opus 5 · Sonnet 5 · Fable 5)
>
> | 버전 | 날짜 | 변경 내용 |
> | --- | --- | --- |
> | v1.0 | 2026-07-03 | 8개 소스 통합 초판 |
> | v1.1 | 2026-07-03 | 공식 재검증: deny 파일명령 감지, `attribution` 교체, PowerShell deny 패턴, Windows sandbox 미지원, deny→ask→allow 우선순위 |
> | v1.2 | 2026-07-08 | 추론 강도 제어 `/effort` §D-6 신설 |
> | v1.3 | 2026-07-20 | 전면 재검증: 샌드박스 Linux·WSL2 지원, Read deny의 Edit 차단, PowerShell 별칭 자동 정규화, frontmatter `effort:` 확정, `/effort auto`·`ultracode`, `.claude/rules/` 소개 |
> | v1.4 | 2026-07-20 | §F-1 **검증 기준 규칙**(성공 기준·검증 명령 동봉), §E `/init`, 공유 목록에 `.mcp.json`, §G plan mode·`/clear` 습관 |
> | v1.5 | 2026-07-23 | 재검증(v2.1.218): 샌드박스 읽기 정책 표현 정정, `sandbox.filesystem.disabled`, `Tool(param:value)` 매칭. 범용성 정리(프로젝트명·개인 취향 키 제거) |
> | v1.6 | 2026-07-23 | **기본 원칙 명문화**(§0): 1순위=최적 결과물, 절약=품질 유지 전제의 2순위 |
> | v1.7 | 2026-07-23 | (구)02 가이드 제거 — 세션 운영 상세는 공식 문서 직접 참조. 문서 번호 재편(이 문서 00→01) |
> | v1.8 | 2026-07-28 | Opus 5 반영 — §D-6 effort 지원표 갱신 |
> | v1.9 | 2026-07-28 | **부분 선택 도입**(STEP 3 구성 항목 선택 목록) + **§J-1 auto mode 신설** |
> | v1.10 | 2026-07-28 | **절약 프로필**(§D-6): medium 시작·필요 시만 상향, 품질 불변 지점 명시 |
> | v1.11 | 2026-08-04 | 적용 기록 한 줄 추가(STEP 4 `dropin-applied` — 형식은 00 §A) |
> | v1.12 | 2026-08-04 | 전면 재검증(v2.1.221) — 전 서술 유효 확인. "최근 재검증" 표기 분리 |
> | v1.13 | 2026-08-10 | **/resume 원격 최신화 선행**(§F-1·§F-2 A/B/C): remote 있고 워킹트리 clean이면 `git pull --ff-only` 먼저, 아니면 fetch 후 뒤처짐만 보고 |
> | v1.14 | 2026-08-10 | §E `/init` 대화형 플로우, §K `/doctor` 행. 커스텀 /resume·/wrap+docs/ 기록 체계는 내장 Tasks 대비 정본 유지 |
> | v1.15 | 2026-08-10 | §F-2 설치 전 **같은 이름 구형 `commands/` 제거** 규칙 + §K 증상 행 |
> | v1.16 | 2026-08-10 | **`/dropin-check` 배포**(§D-4) + /resume 조건부 안내(적용 30일 경과·PROJECT_PLAN 예정일 경과 시만) |
> | v1.17 | 2026-08-10 | **`/dropin-update` 신설**(§D-4): 승인한 문서만 최신본 교체, 원본 repo에선 git pull 안내 후 중단 |
> | v1.18 | 2026-08-10 | 적용 기록 표기 자기 참조화(버전 하드코딩 제거) + STEP 3 ⓒ에 점검 스킬 병기 |
> | v1.19 | 2026-08-10 | /resume의 **PROGRESS 앞 ~40줄만 Read limit**(§F-1·§F-2 A/B/C) — 통째 읽기 차단 |
> | v1.20 | 2026-08-12 | 점검·최신화 스킬 **설치 유형 전 지원** 명시(사본/클론/raw — 00 `출처=` 필드로 판별) |
> | v1.21 | 2026-08-12 | 조건부 안내 2건(§F-2 A/B/C): /resume 조치 대기 메모 안내, /wrap의 PROGRESS ~800줄 경계 감지·아카이브 안내 |
> | v1.22 | 2026-08-12 | **아카이브 포인터 최상단**(제목 바로 아래 — /resume 40줄 읽기에 보이게) + 과거 이력 검색은 `docs/` 폴더 단위 |
> | v1.23 | 2026-08-12 | 스킬 스코프 우선순위 **정정**(**enterprise > personal > project**, 스킬>커맨드) — §D-4·§E-3·§E-4·§F-2 통일, B·C는 글로벌 기본형(A) **교체** 방식, 중첩 스킬 디렉터리-한정 이름(v2.1.203+) |
> | v1.24 | 2026-08-12 | §D-3 deny 예시 실물 동기화+선택 확장 예, §F-2 A/C resume에 최근 DEC 3건, §G pull 이중 실행 제거, §F-1 append는 포인터 아래부터, §B 비-git 행, §K push 권한 없음 행, A wrap 파생 뷰 동기화 |
> | v1.25 | 2026-08-12 | **/wrap에 글로벌 정본↔백업 미러 대조 배선**(§F-2 A/B/C): 세션 중 수정 시만 5종 해시 대조 → 방향 판정+복사 안내. `settings.json`은 키 단위·머신 종속 키 제외 |
> | v1.26 | 2026-08-12 | §F-2 B·C **교체의 PC 전역 전파 경고** + resume·wrap 템플릿 4곳에 **폴백**(단위 없음·단일 repo·비-git) 의무화 |
> | v1.27 | 2026-08-12 | §D-2 "정본은 `global-config/CLAUDE.md`" 포인터, §D-1 폴더 4종, §A STEP 3 모드별 ⓐ~ⓕ 기본값 매핑, 오참조·hooks 예시·버전 표 순서 정정 |
> | v1.28 | 2026-08-12 | §F-2 A wrap 판정 기준 **4키+포함 기준 정본**, resume 3곳 `게이트 차단(사유)` 소비자 배선, **최소 모드=ⓑⓒⓓ** 정정, §D-3 deny 배열 순서 무관 명시 |
> | v1.29 | 2026-08-12 | §F-2 A 비-git 폴백·B/C wrap 4키 참조·전 템플릿 `docs/` 부재 폴백, §L 점검 범위 A·B·C 전 템플릿, STEP 3 글로벌 대상은 ⓓⓔ 제외, §F-1 `[Pending]`·`[Blocked]` PROJECT_PLAN 승격 규칙+정기 항목 `다음 ○○: YYYY-MM경` 형식, 역방향 교체 경고 |
> | v1.30 | 2026-08-12 | §F-2 resume 3곳 `소스없음`/`확보 실패` 어휘+글로벌 `CLAUDE.md` 읽기, A wrap 줄바꿈 정규화, `docs/` 부재 폴백 위치 정정, §L 조건부 안내 어휘 동기화 |
> | v1.31 | 2026-08-12 | §B **혼합형** 행 신설(독립 repo+비-git 혼재 — 단위별 git 판정), §F-2 C wrap 루트 라우터 예외, A 템플릿 세션 기록 소실 고지 |
> | v1.32 | 2026-08-12 | §A STEP 1 유형 열거 제거(§B 표를 가리킴 — 행이 늘어도 낡지 않게), 루트 라우터 예외 조건을 "PROGRESS·DECISIONS·PROJECT_PLAN 부재"로 정정 |
...
**문서 정보** — 통합 마스터(범용) **v1.46**. 변경 이력은 최상단 버전 표 참조(유래: 8개 소스 통합 초판 — v1.0 행).
