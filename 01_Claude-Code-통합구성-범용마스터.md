...
- **절약 프로필** (Pro 요금제·한도 관리 사용자 — 00 STEP 3에서 선택): 절약은 품질 유지 전제의 2순위이므로 **모델 배정 하향 + 낭비 제거**로만 아낀다(effort는 프로필로 갈리지 않는다 — 아래).
  ...
  - **줄이지 않는 것**(품질 불변): 트리아지·spec·최종 리뷰의 상위 모델(02 §F), 검증 게이트(04) — 여기서 아끼면 재작업이 더 비싸다.
...
- 커밋: `git add CLAUDE.md docs .claude .gitignore .gitattributes`(있는 것만 — 없는 경로 하나로 `git add` 전체가 실패한다) → 한 repo에서 커밋.
...
### 동시편집 충돌
- PROGRESS·DECISIONS는 append 전용(기존 줄 수정 금지 — 예외는 아래 되돌림·Superseded **표시** 한 조각뿐이고, 병합 뒤 같은 항목이 표시 유무로 두 벌이면 표시 없는 쪽을 지운다 — union은 충돌 없이 둘 다 남긴다). .gitattributes의 merge=union이 동시 append를 자동 합침.
