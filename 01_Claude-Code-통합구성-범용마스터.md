...
> **문서 버전: v1.31** · 최종 갱신: **2026-08-12** · **최근 재검증: 2026-08-04** · 기준: Claude Code v2.1.226 (Opus 5 · Sonnet 5 · Fable 5)
>
> | 버전 | 날짜 | 변경 내용 |
> | --- | --- | --- |
...
---
# /wrap (통합)
1) 각 repo git status로 변경 감지(**폴백**: 단일 repo면 그 repo만 — 3)·4)는 생략해 A와 동일. 비-git 폴더면 git 단계를 건너뛰고 기록만). 2) 변경 repo마다 docs[/<단위>]/PROGRESS.md 최상단 append(**폴백**: 그 단위에 `docs/`가 없으면 append하지 말고 "01 §E-1 기록 체계 미설치"를 알린다. **루트는 예외** — 얇은 라우터라 `docs/INDEX.md`만 있는 것이 정상이니 미설치로 보지 않는다).
...
**문서 정보** — 통합 마스터(범용) **v1.31**. 변경 이력은 최상단 버전 표 참조(유래: 8개 소스 통합 초판 — v1.0 행).
