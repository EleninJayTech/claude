...
- 로컬 클론이면 점검 전 `git fetch`로 **뒤처짐(behind 커밋 수)만 병기**한다 — 낡은 기준으로 "최신" 오보를 막되, 이 스킬은 읽기 전용이라 `pull`은 하지 않는다(사용자가 그 클론에서 편집 중일 수 있다). 뒤처져 있으면 "`/dropin-update`로 클론 최신화 후 재점검"을 권고한다.
...
- 마지막 줄 권고: 문서 차이 → "로컬 문서 사본이 구버전이면 `/dropin-update`로 먼저 갱신 → 00 문서 재적용(재적용 분기가 버전 diff를 안내)" / 글로벌 차이 → **§3에서 판단한 방향대로 2분기**: 로컬(`~/.claude`)이 구버전이면 "`global-config/`에서 해당 파일 복사", 백업이 구버전이면 "원본을 `global-config/`로 복사해 백업 갱신·커밋"(최신 로컬을 구백업으로 덮지 않는다). **복사를 담당하는 전용 스킬은 없다** — 이 스킬은 읽기 전용이고 `/dropin-update`는 문서 파일만 다루므로, 사용자가 원하면 **이 세션에서 승인받아 복사하고 `/wrap`의 미러 대조로 확인**한다(방치하면 다음 세션에 같은 보고가 반복된다). `commands`↔`skills` 이름 중복도 같은 방식(삭제는 승인 후) / 적용 기록 없음 → "00 재적용으로 기록 수립 권장"(흔적 감지 한계 해소) / 전부 일치 → "최신 — 조치 불요".
