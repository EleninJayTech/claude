...
- 로컬 클론이면 점검 전 `git pull --ff-only`(clean일 때)로 기준을 최신화. **더티면 pull 대신 `git fetch`로 뒤처짐(behind 커밋 수)을 병기** — 낡은 기준으로 "최신" 오보를 막는다.
- **오프라인이고 로컬 클론도 없으면**(원격 조회 실패) "점검 불가"로 보고하고 종료한다 — 추측 비교 금지.
- 각 드롭인 문서(`NN_*.md` 전부, 현재 00~06)의 **최상단 헤더+버전 표만** 읽어 `문서 버전: vX.Y`와 §4 인용 재료(해당 버전 행)를 추출(전체 Read 금지 — 앞 ~30줄이면 충분).
...
- 마지막 줄 권고: 문서 차이 → "로컬 문서 사본이 구버전이면 `/dropin-update`로 먼저 갱신 → 00 문서 재적용(재적용 분기가 버전 diff를 안내)" / 글로벌 차이 → **§3에서 판단한 방향대로 2분기**: 로컬(`~/.claude`)이 구버전이면 "`global-config/`에서 해당 파일 복사", 백업이 구버전이면 "원본을 `global-config/`로 복사해 백업 갱신·커밋"(최신 로컬을 구백업으로 덮지 않는다) / 적용 기록 없음 → "00 재적용으로 기록 수립 권장"(흔적 감지 한계 해소) / 전부 일치 → "최신 — 조치 불요".
