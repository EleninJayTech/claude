---
name: skill-setup
description: 후보 스킬 카탈로그에서 이 프로젝트에 맞는 것만 선별 설치(계획 승인 게이트·멱등·CLAUDE.md 관리 블록 기록). 수동 호출 전용.
...
정체는 반드시 실행 시점에 직접 확인할 것 — 카탈로그의 외부 URL은 생존 점검을 `/reverify` §1-C가 맡고
정체 확인은 실행 시점 몫이다. 404·저장소 개편이면 힌트를 갱신하거나 항목을 제외한다.)*
...
3. **frontend-design** — 뻔한 AI 디자인(흰 배경·보라 그라데이션·단조 레이아웃) 금지.
   ...
   *(표준 스킬 폴더 — 경로는 저장소 개편이 잦으니 실행 시점에 위치 확인. **단, Claude Code에 이미 내장**되어
   있을 수 있음 → 중복이면 제외. UI/프론트엔드가 있는 프로젝트에만 적합.)*
...
7. **humanizer** — AI 특유의 작위적 문체 배제 + 사용자의 글쓰기 습관(리듬·단어 선택) 반영 교정.
   ...
   *(저장소 표기는 단일 `SKILL.md`지만 참조 파일이 딸릴 수 있다 — `SKILL.md`가 직접 든 폴더째 복사.
   한국어 사용자 대상 글/문서/카피 작성이 있으면 유용.)*
...
- 스킬 로드 확인(`/` 자동완성 목록·`/context`), 플러그인 목록 확인(`/plugin`). 세션이 새 스킬을 인식하려면 재시작이
  필요한 경우 그 사실을 안내한다.
