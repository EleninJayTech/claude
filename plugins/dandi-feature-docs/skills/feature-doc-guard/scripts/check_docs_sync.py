...
def main():
    ...
    if unmapped:
        print("[feature-doc-guard] 기능 문서에 매핑되지 않은 소스 (새 기능이면 /dandi-feature-docs:feature-docs sync):")
        for p in unmapped:
            print("   -", p)
    if missing:
        ...
        print("   문서를 갱신하고 함께 add 하거나, /dandi-feature-docs:feature-docs sync 를 실행하세요.")
