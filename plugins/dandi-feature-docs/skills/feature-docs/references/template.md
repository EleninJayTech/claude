...
- 기능 문서를 하나라도 고치면 VERSION.md 의 docs_version 을 올리고 history 에 추가한다.
  ...
  뒤처진 문서를 목록으로 보고한다.
- 커밋 전 `/dandi-feature-docs:feature-docs sync`, 정기적으로 `/dandi-feature-docs:feature-test` 를 실행한다.
- 주기 점검은 `/dandi-feature-docs:feature-doc-audit`(문서 상태만), 커밋 가드는 `/dandi-feature-docs:feature-doc-guard install`,
  무인 실행용 스크립트는 `/dandi-feature-docs:test-to-script`, 배포 시 `/dandi-feature-docs:release-notes` 를 쓴다 (설치된 경우).
- `/dandi-feature-docs:feature-test` 결과가 (B) 문서 노후로 판정되면 사용자 승인 후 `/dandi-feature-docs:feature-docs sync` 로 고친다.
```
