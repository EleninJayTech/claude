...
`docs/features/03_test_index.md` 가 없으면 "먼저 `/dandi-feature-docs:feature-docs init`" 을 안내하고 중단한다.
...
- 만든 스크립트를 실제로 한 번 실행한다. 실패하면 스크립트가 틀린 것인지 문서가 틀린 것인지
  판정해 보고한다. 문서가 틀렸으면 고치지 말고 `/dandi-feature-docs:feature-docs sync` 를 안내한다.
...
- 무인 정기 실행은 이 명령을 작업 스케줄러·CI 에 직접 걸거나 `claude -p "/dandi-feature-docs:feature-test"` 로 감싼다.
