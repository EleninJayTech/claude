...
| 파일 | 역할 | 버전 |
| --- | --- | --- |
| `00_통합-설치.md` | 01~06 통합 설치 셀렉터 — 문서 확보→감지→설치 모드(최소/권장/전체/사용자)+목적 번들(§C-2 합집합)+운용 프로필(표준/절약)→해당 문서 프로토콜로 위임, 적용 기록(`dropin-applied` — `출처=` 포함)·재적용 분기 | v1.17 (2026-08-12) |
| `01_Claude-Code-통합구성-범용마스터.md` | 글로벌+프로젝트 셋업 마스터(§D 글로벌·§E 환경별·§F SSOT 블록·§J-1 auto mode·§D-6 절약 프로필) — 구성 항목 부분 선택 | v1.30 (2026-08-12) |
| `02_모델분담-범용-플레이북.md` | 상위/하위 모델 자동 분담, `.claude/agents/` 로스터 템플릿(에이전트별 선택, 리뷰 정본=내장 `/code-review`, §F 절약 배정=Sonnet+advisor 패턴, 비개발 repo 가드) | v1.19 (2026-08-12) |
| `03_확장기능-설치-체크리스트.md` | 확장 5종(claude-hud·svg-design·frontend-design·MCP·브라우저 검증) 체크리스트 설치 + 플러그인 부록 | v1.12 (2026-08-12) |
| `04_검증-자동화.md` | 검증 3종(hooks 게이트·gh/glab CLI·CI 통합) — git 호스트 감지 분기, 회사 정책 확인 게이트 | v1.9 (2026-08-12) |
| `05_출력스타일.md` | 응답 역할·톤·형식 커스텀 — 내장 스타일 4종 전환 + 커스텀 `output-styles/*.md` 생성(`keep-coding-instructions` 함정 안내) | v1.6 (2026-08-12) |
| `06_루틴-자동화.md` | 클라우드 스케줄 에이전트(스케줄·API·GitHub 트리거) — repo 준비물+루틴 생성+정의 기록(docs/ROUTINES.md). Pro+ · research preview · 회사 게이트 | v1.7 (2026-08-12) |
| `index.html` | 사람용 안내 페이지(입문자 대상 사용법, GitHub Pages 랜딩) — 드롭인 아님, 설치 무관. **루트 고정**(Pages 진입점) | 2026-08-12 |
| `pages/demo.html` | 사람용 설치 시연 페이지(터미널 재생형 인터랙티브 — 재생/챕터/배속/자막) — index.html과 동일 규칙(사람용 예외, 절차가 크게 바뀔 때만 갱신) | 2026-08-12 |
| `pages/intro.html` | 사람용 1분 소개 영상 페이지(16:9 시네마틱 7장면 자동 전환 — 유튜브 소개용, 화면 녹화 시 영상화) — 사람용 예외 동일 | 2026-08-12 |
| `pages/options.html` | 사람용 설치 옵션 사전(00 설치 중 나오는 모든 선택 항목을 입문자 눈높이로 설명 — 용어 8개·공통 질문 4종·01~06 항목별·추천 조합·FAQ) — 사람용 예외 동일 | 2026-08-12 |
| `pages/learn.html` | 사람용 설치 옵션 학습 코스(챕터 11개 순차 진행 + 선택 시뮬레이터·진행도 localStorage 저장 — 옵션 사전의 코스형 자매 페이지) — 사람용 예외 동일 | 2026-08-12 |
| `global-config/` | `~/.claude` 백업(화이트리스트 5종) — 새 PC 복원용. **파일마다 마지막 복사 시점이 다르다**(한 번에 전부 재복사하지 않으므로) — 아래 날짜는 이 폴더의 최종 갱신일이지 전 파일의 공통 스냅샷 시점이 아니다 | 2026-08-12 최종 갱신 |
