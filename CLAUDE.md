...
| 파일 | 역할 | 버전 |
| --- | --- | --- |
| `00_Claude-Code-통합구성-범용마스터` | 글로벌+프로젝트 셋업 마스터(§D 글로벌·§E 환경별·§F SSOT 블록) | v1.4 (2026-07-20) |
| `01_모델분담-범용-플레이북` | 상위/하위 모델 자동 분담, `.claude/agents/` 로스터 템플릿 | v1.3 (2026-07-20) |
| `02_진단-확장활용-가이드` | 세션 운영 기법(검증 루프·plan mode·컨텍스트 관리·worktree·헤드리스·MCP) | HTML만 |
| `global-config/` | `~/.claude` 백업(화이트리스트 4종) — 새 PC 복원용 | 2026-07-20 스냅샷 |

## 문서 관리 규칙 (이 repo 고유)
- **md + html 이중 관리**: md는 Claude 학습용 드롭인, html은 사람용. 역할이 다르므로 중복을 감수하고 **둘 다 동기화**한다. ([[DEC-20260720-bsjeong87-01]])
