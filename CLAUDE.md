...
| `01_Claude-Code-통합구성-범용마스터.md` | 글로벌+프로젝트 셋업 마스터(§D 글로벌·§E 환경별·§F SSOT 블록·§J-1 auto mode·§D-6 절약 프로필) — 구성 항목 부분 선택 | v1.53 (2026-08-23) |
...
| `pages/options.html` | 사람용 설치 옵션 사전(00 설치 중 나오는 모든 선택 항목을 입문자 눈높이로 설명 — 용어 8개·공통 질문 4종·01~06 항목별·추천 조합·FAQ) — 사람용 예외 동일 | 2026-08-23 |
...
<!-- dropin-applied: 2026-08-12 · 모드=권장+무인자동화 · 프로필=표준 · 01 v1.26(재구성 점검 — §F-1 길이 관리 문구를 정본으로 동기화 1건, 그 외 ⓐ~ⓕ 어긋남 0. **v1.26 재점검 2026-08-12**: §F-2 B·C 전파 경고·폴백 델타는 이 repo가 A형이라 영향 0, ⓒ 글로벌 스킬 2종 어긋남은 그 PC에서 해소) · 06 v1.3(§2·§4 준비물 — docs/ROUTINES.md 틀+.gitattributes merge=union / **미선택: 06 §3 루틴 생성**(사내 도입 예정 없음)) · 미선택: 02·04(해당 없음-문서 repo — 빌드·테스트가 없어 hooks 게이트를 걸 대상이 없음. 04 채택 시 호스트 CLI 인증 필요하나 **설치·인증 상태는 PC마다 달라 각 `~/.claude/CLAUDE.md` 글로벌 기록에서 확인**), 03·05(글로벌 기적용) · 출처=사본(이 repo가 문서 원본 — /dropin-update는 git pull 안내 후 중단) -->
